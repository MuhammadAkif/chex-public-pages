...
export function LocationRegions({
  ...
  return (
    <section
      className={['bg-white px-4 py-20 sm:px-6 lg:px-10 lg:py-24', sectionClassName]
        .filter(Boolean)
        .join(' ')}
    >
      <div className="mx-auto max-w-[1240px]">
        <div className={['mx-auto text-center', headingClassName].filter(Boolean).join(' ')}>
          <h2
        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-2">
          {items.map((item) => (
            <article
              key={item.city}
              className={['grid overflow-hidden rounded-[16px] bg-[#f0f6ff]', articleClassName]
                .filter(Boolean)
                .join(' ')}
            >
              ...
              <div className="flex flex-col justify-center p-7 sm:p-9">
                ...
                <a href={demoHref} className="type-location-body mt-5 font-medium underline">
                  Learn more
                </a>
              </div>
            </article>
